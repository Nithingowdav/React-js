...
class UserClass extends React.Component {
  ...
  render() {
   // const { name, location } = this.props; // Destructuring props for cleaner code
    ...
    const { name, login, user_view_type, html_url , avatar_url} = this.state.userInfo; // Destructuring state for cleaner code
    return (
      ...
      <div className="user-card">
       ...
       {/* <h1>count : {count}</h1>
        ...
        <img src={avatar_url} alt="User Avatar" style={{ width: "100px", height: "100px" }} />
        <h1>Name: {name}</h1>
        <h2>login: {login}</h2>
